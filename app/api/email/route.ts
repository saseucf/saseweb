...
export async function POST(req: NextRequest) {
    try {
                ...
                html: `
                    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
                        ...
                        <div style="padding: 32px; background-color: #ffffff;">
                            <h2 style="color: #171d52; margin-top: 0;">Welcome to UCF SASE! 🎉</h2>
                            <p style="color: #4b5563; font-size: 16px; line-height: 1.5;">
                                Hi${nameDisplay},
                            </p>
                            <p style="color: #4b5563; font-size: 16px; line-height: 1.5;">
                                Your account has been successfully created. We're excited to have you as part of the UCF Society of Asian Scientists & Engineers community!
                            </p>
                            <p style="color: #4b5563; font-size: 16px; line-height: 1.5;">
                                Stay tuned for upcoming events, workshops, and opportunities.
                            </p>
                        </div>
                        <div style="background-color: #f9fafb; padding: 16px; text-align: center;">
                            <p style="color: #9ca3af; font-size: 12px; margin: 0;">UCF SASE · University of Central Florida</p>
                        </div>
                    </div>
                `,
            });
        ...
        // ── RSVP confirmation email (default) ─────────────────────────────────
        if (!eventTitle) {
            return NextResponse.json(
        ...
        if (error) {
            console.error("Resend error (rsvp):", error);
            return NextResponse.json({ error: error.message }, { status: 500 });
        }

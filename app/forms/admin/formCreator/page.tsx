...
function FormCreator() {
  ...
  useEffect(() => {
    async function loadForm() {
      ...
      const hasNameField = data.schema?.some((q: Question) => q.label?.toLowerCase() === "full name");
      if (hasNameField) setCollectGuestName(true);
    }
  ...
  function handleCollectGuestNameChange(checked: boolean) {
    setCollectGuestName(checked);
    if (checked) {
      // Add Full Name question if it doesn't exist
      if (!questions.some(q => q.label?.toLowerCase() === "full name")) {
        ...
        setQuestions([nameQuestion, ...questions]);
      }
    } else {
      // Remove Full Name question
      setQuestions(questions.filter(q => q.label?.toLowerCase() !== "full name"));
    }
  }
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className="relative flex items-center">
                <input
                  ...
                  onChange={(e) => handleCollectGuestNameChange(e.target.checked)}
                  className="peer sr-only"
                />
                <div className="w-5 h-5 rounded-md border-2 border-border peer-checked:border-[#89abe3] peer-checked:bg-[#89abe3] transition-all flex items-center justify-center">
                  <svg className="w-3 h-3 text-white opacity-0 peer-checked:opacity-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
              ...
              <span className="text-sm font-semibold text-foreground group-hover:text-[#89abe3] transition-colors">Collect Guest Name</span>
            </label>
          )}
        </div>
      </div>
